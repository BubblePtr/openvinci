...
The prompt comes from the positional argument, or from stdin when there is no positional argument (handy for long prompts full of quotes and newlines). Passing both, or neither, is a usage error.
...
| `--json` | off | Print a result object on success, an error object on failure. |
| `-h`, `--help` | | Show usage. |
...
`size` echoes the requested size (`auto` when you did not ask for one).
...
| --- | --- | --- |
| `usage_error` | 2 | Bad flags, a missing prompt, a prompt on both stdin and argv, or a format conflict. |
